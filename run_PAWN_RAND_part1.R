...
## CASE = with geostratum is considered a cluster
CASE = CASE.list[it,1]

## set the name for storage
nom = paste0(resfolder,"/FIXE_CLUST_N",NN,"_nc",250,"NumC_",1,"_CASE",CASE,"_RAND")

## generate
SEED = SEEDr[it]
source("./utils/run_extract_rand_Europe.R")

## define the training dataset
load(paste0(outfolder,"OCSdata_rand_train.RData"))
...
## define the geographica coordinates
XY.te = df.te[,c(1,2)]
...
#### ---------------------------------------
ncv = 10
## Define here the levels of the random variables
...
for (ii in 1:nmc){
	...
	if (rerr[ii] == "N"){
		...
		w = sample(c(1,1/4),nrow(dfB),replace=TRUE)#rep(1/5,nrow(dfB))
	}
