...
N.list = c(250) ##number of observations within the cluster = dummy variable for random case

outfolder = paste0("./resu_EUR/",CASE0,"/")
resfolder = paste0("./resu_EUR/",CASE0,"/RAND/")
...
load(paste0("./data_EUR/",CASE0,"/OCSdata_rand_train.RData"))
...
for (it in 1:IT){
	...
	CASE = CASE.list[it,1]
	nom = paste0(outfolder,"FIXE_CLUST_N",NN,"_nc",N,"NumC_",Nc,"_CASE",CASE,"_RAND") ## name where load data
